...
// Function for formatted output of all stripes (ordered, through root)
void ParallelPrintMatrix(int *pProcRows, int Size, int RowNum)
{
    ...
    if (ProcRank == 0)
    {
        ...
        for (int i = 0; i < ProcNum; i++)
        {
            recvCounts[i] = (restRows / (ProcNum - i)) * Size;
    ...
    MPI_Gatherv(pProcRows, RowNum * Size, MPI_INT,
    ...
    if (ProcRank == 0)
    {
        ...
        for (int i = 0; i < ProcNum; i++)
        {
            int localRows = recvCounts[i] / Size;
            printf("\nProcRank = %d\n", i);
            printf("Proc rows:\n");
            PrintMatrix(fullMatrix + displs[i], localRows, Size);
        }
